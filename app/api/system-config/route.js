...
export async function GET() {
  try { let config = await prisma.systemConfig.findFirst()
    if (!config) {
...
export async function PATCH(request) {
  try { const { allWfaActive } = await request.json()
    const updated = await prisma.systemConfig.updateMany({
      data: { allWfaActive },
    })

    if (updated.count === 0) { await prisma.systemConfig.create({ data: { allWfaActive } })}

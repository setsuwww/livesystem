...
export function Calendar({ events, onDateClick, onEditEvent, onDeleteEvent }: Props) {
  return (
    <FullCalendar plugins={[dayGridPlugin, interactionPlugin]}
      initialView="dayGridMonth" height="auto" selectable
      ...
      eventContent={(eventInfo) => (
        <div className="flex items-center justify-between border border-gray-300 shadow-sm w-full rounded-lg bg-white px-2 py-2 hover:shadow-lg transition">
      ...
      dayCellClassNames={(arg) => {
        if (dayjs(arg.date).isSame(dayjs(), "day")) {
          return "bg-sky-100 border border-sky-400"; 
        }

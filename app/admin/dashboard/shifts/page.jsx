...
import { ShiftType } from "@prisma/client"; // <- import enum
...
async function getMainShifts() {
  return await prisma.shift.findMany({
    where: { type: { in: [ShiftType.MORNING, ShiftType.AFTERNOON, ShiftType.NIGHT] } }, // <- enum
    select: {
      id: true,
      type: true,
      startTime: true,
...
async function getCustomShifts(page = 1) {
  const [shifts, count] = await Promise.all([
    prisma.shift.findMany({
      where: { type: ShiftType.CUSTOM }, // <- enum
    ...
    prisma.shift.count({ where: { type: ShiftType.CUSTOM } }),
  ]);

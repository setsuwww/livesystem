      ...
      {options.map((option) => {
        ...
        return (
          <label key={option.value} className={cn("cursor-pointer rounded-md border px-3 py-2 text-sm font-medium flex flex-col", "duration-300 ease-in-out active:scale-95",
            selected ? "border-gray-400 bg-gray-50 text-gray-700"
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={cn("w-5 h-5 flex items-center justify-center rounded-full border", selected ? "border-gray-700" : "border-gray-400")}>
                {selected && <span className="w-3 h-3 rounded-full bg-gray-700"></span>}
              </span>
